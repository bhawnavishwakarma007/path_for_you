...
const MasterEditJob = () => {
    ...
    const fetchJob = async () => {
      try {
        ...
        setJobDetails({
          ...
          jobType: job.jobType || '',
          applyStartDate: job.applyStartDate?.split('T')[0] || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Navigate back up to /master/jobsmanagement
      navigate('/master/jobsmanagement');
    } catch (err) {
      console.error('Update job error:', err.response?.data || err.message);
      setErrorMsg('Something went wrong while updating the job.');
    } finally {
  ...
  if (loading) {
    return <p className="jobs-management__loading">Loading job details...</p>;
  }
